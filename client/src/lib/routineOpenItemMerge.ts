...
import { computeNextOccurrence } from './rruleUtils';

dayjs.extend(utc);

// MIRROR: api-server/src/lib/routineOpenItemMerge.ts — keep the stamping rules, merge semantics,
// and first-occurrence anchor logic in sync. Both sides run the same test-vector matrix
// (src/tests/routineOpenItemMerge.test.ts / src/tests/routineOpenItemMerge.test.ts).
...
/** The generator-stamped slice of an open item: what `persistRoutineItem` writes from the routine. */
export type StampedItemContent = Pick<StoredItem, 'title' | OptionalStampedKey>;

/**
 * The exact content `persistRoutineItem` (client) / `buildRoutineItemSnapshot` (server) would stamp
 ...
 */
export function stampedItemContent(routine: Pick<StoredRoutine, 'title' | 'template'>): StampedItemContent {
...
function applyAdoptedValue<K extends OptionalStampedKey>(merged: Pick<StoredItem, OptionalStampedKey>, key: K, value: StampedItemContent[K]): void {
    if (value === undefined) {
        delete merged[key];
        return;
    }
...
export interface RoutineContentEdit {
    previous: Pick<StoredRoutine, 'title' | 'template'>;
    next: Pick<StoredRoutine, 'title' | 'template'>;
    now: string;
}
 ...
 * field changed so callers can skip the write + sync op entirely.
 */
export function mergeRoutineEditIntoOpenItem(item: StoredItem, edit: RoutineContentEdit): StoredItem | null {
    const oldStamp = stampedItemContent(edit.previous);
    const newStamp = stampedItemContent(edit.next);
    const merged: StoredItem = { ...item };
 ...
 * `createFirstRoutineItem` / `ensureFirstRoutineItem`, extracted so the schedule-recompute path
 * produces the same date the generator would. `localTodayStr` is the caller's local calendar date
 * (see `createFirstRoutineItem` for the UTC-midnight-on-local-date reasoning); passing it in keeps
 * this pure and testable. Throws `RruleExhaustedError` when the rrule has no occurrence left.
 */
export function computeFirstOccurrenceDate(routine: Pick<StoredRoutine, 'rrule' | 'startDate'>, localTodayStr: string): string {

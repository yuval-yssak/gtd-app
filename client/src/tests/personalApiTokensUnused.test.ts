...
describe('isTokenUnused', () => {
    it('returns true when lastUsedTs is missing — never-used tokens are the safest to revoke', () => {
        expect(isTokenUnused({ id: '1', label: 'a', createdTs: '2026-01-01T00:00:00.000Z' }, now)).toBe(true);
    });
    ...
    it(`returns true when lastUsedTs is older than ${UNUSED_THRESHOLD_DAYS} days`, () => {
        ...
        expect(isTokenUnused({ id: '1', label: 'a', createdTs: '2026-01-01T00:00:00.000Z', lastUsedTs: cutoff }, now)).toBe(true);
    });
    ...
    it(`returns false when lastUsedTs is exactly under the ${UNUSED_THRESHOLD_DAYS}-day window`, () => {
        ...
        expect(isTokenUnused({ id: '1', label: 'a', createdTs: '2026-01-01T00:00:00.000Z', lastUsedTs: recent }, now)).toBe(false);
    });

    it('returns false when the token is revoked (no point chipping it as unused)', () => {
        const veryOld = now.subtract(365, 'day').toISOString();
        expect(
            isTokenUnused({ id: '1', label: 'a', createdTs: '2025-01-01T00:00:00.000Z', lastUsedTs: veryOld, revokedTs: '2026-04-01T00:00:00.000Z' }, now),
        ).toBe(false);
    });
    ...
    it('uses ≥ comparison: a token whose lastUsedTs is exactly the threshold is unused', () => {
        ...
        expect(isTokenUnused({ id: '1', label: 'a', createdTs: '2026-01-01T00:00:00.000Z', lastUsedTs: exactlyThreshold }, now)).toBe(true);
    });
});

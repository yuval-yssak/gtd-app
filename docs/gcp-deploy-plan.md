...
- **Deploys are two steps, and the second is required**: the workflow runs `gcloud run deploy --no-traffic` and then promotes with `gcloud run services update-traffic --to-latest`. `--no-traffic` pins traffic to the revision that was live before the deploy and turns off the service's float-to-LATEST behaviour, so if the promote step is ever removed or skipped the service keeps serving the *old* revision indefinitely while the job still reports success. A final step verifies the serving revision matches the latest ready one, so a stalled rollout fails the job. The split exists because `--max-instances=1` means a new revision cannot start until the old instance drains, and open SSE streams can hold that off for tens of minutes — a deploy on 2026-09-22 took 59m and `gcloud run deploy` timed out at ~56m even though the revision was healthy. See also the 120s SSE stream lifetime cap in `api-server/src/routes/sync.ts`, which bounds how long a tab can keep an instance busy.

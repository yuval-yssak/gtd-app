...
Endpoint tables, the status→Google-effect matrix and the escalation state machine live in [`README.md`](README.md) § Calendar Integration. Rules to keep when touching `src/lib/calendarPushback.ts` or the inbound exception pipeline in `src/routes/calendar.ts`:

- `maybePushToGCal` is fire-and-forget from `lib/notifyChange.ts`. Every branch of `handleItemPush` / `handleRoutinePush` is wrapped by `captureFailedOutcome`; a provider error must end as `markOpFailed(opId, reason, detail)` (`lib/opFailure.ts`, reason from `lib/gcalErrorCategorization.ts`). A branch that returns `undefined` on a condition it cannot satisfy is a **silent drop** — the user gets no SyncIssuesPanel row and nothing retries. `resolvePushContext` does exactly this for a `suspended`/`revoked` integration: it logs `skipping <status> integration` and returns `null`, and every caller returns `undefined` without marking the op.
- Never patch or delete a series **master** from an item push — reroute to a single-instance override. Instance pushes pass `instanceEventId: item.calendarInstanceEventId`; the date-window `events.instances` fallback exists only for legacy rows without one.
- Done keeps the event (`applyDoneMarker` + `DONE_COLOR_ID` in `lib/doneMarker.ts`); trash deletes/cancels. Item creates and updates (standalone and instance overrides) take `sendUpdates` from `op.gcalMeta` (default `'none'`); instance cancellations always send `'none'`, standalone deletes send no `sendUpdates` parameter at all, sweep re-pushes hard-code `'none'`, and routine-series pushes never read `gcalMeta`.
- `pushRoutineInstanceOverride` always sends `timeStart`/`timeEnd`. Re-pushing a stale row (e.g. from a sweep) moves the Google occurrence back — only send times when the row was locally moved.
- The missed-push sweep (`runMissedPushSweep`) runs **only** inside `POST /calendar/integrations/:id/sync` (which the client calls on every sync cycle, not just from "Sync now"), after the inbound pass, fenced by `before: <inbound start>`. Candidates must satisfy `isMissedPush` (an anchor is required — an anchor-less row gives no evidence of which side is newer). Every successful push must stamp `lastPushedToGCalTs` or the row re-runs on every sync. Instance pushes also stamp on a 404 because `patchInstanceById` swallows it; a standalone `updateEvent` 404 throws, lands as `failed`, is not stamped, and the row is re-pushed on every sweep.
- Inbound exception targeting (`resolveExceptionTarget`) matches `status: 'calendar'` rows only, in all three tiers, so a Google move or cancellation of an already-`done` occurrence is ignored: the move (unless dated before today, which `isExceptionBeforeToday` drops earlier) falls through to `createItemForOrphanedException`, whose insert collides with the presence-partial `(user, calendarInstanceEventId)` index held by the done row; when that row belongs to the **same** routine the handler bails, when it belongs to a different routine (split successor, re-import) `isDemotableDeadTwin` strips its instance id and a new live row is inserted. The cancellation's `updateItemsAndRecordOps` finds zero ids. Tiers 2/3 are date/instant heuristics and must never claim a done row; only tier 1 (instance id) is a safe key for one.

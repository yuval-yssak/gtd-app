 ...
 * preserves the per-route path layout (`/items`, `/people`, `/work-contexts`, …) while keeping
 * each entity's handlers and middleware self-contained in its own file.
 */
export const v1Routes = new Hono<{ Variables: BearerVariables }>().route('/', v1ItemsRoutes).route('/', v1PeopleRoutes).route('/', v1WorkContextsRoutes);

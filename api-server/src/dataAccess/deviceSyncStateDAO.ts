...
class DeviceSyncStateDAO extends AbstractDAO<DeviceSyncStateInterface> {
     ...
     * Returns `removedDeviceIds` — every deviceId whose row for THIS user was just deleted (the
     * caller drops the matching deviceUsers join rows) — and `fullyDrainedDeviceIds`, the subset
     * with no remaining (device, user) row for ANY user, whose push subscriptions can also go.
     * Devices another account still actively syncs on are never fully drained (a multi-account
     * device shouldn't lose push delivery just because one of its accounts went stale).
     *
     ...
     */
    async deleteStaleDevices(userId: string, cutoffTs: string): Promise<{ removedDeviceIds: string[]; fullyDrainedDeviceIds: string[] }> {
        ...
        const removedDeviceIds = [...new Set(stale.map((d) => d.deviceId))];
        // A device is fully drained only when *every* (device, user) row is gone — i.e. no other
        // user on this device is still active. Otherwise the push subscription is still in use.

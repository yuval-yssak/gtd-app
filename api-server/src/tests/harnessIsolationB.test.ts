...
// Pair with harnessIsolationA.test.ts: both load the SAME logical database name, so only the
...
it("writes a canary only this file can see, and never sees the other file's", async () => {
    ...
    await itemsDAO.insertOne({ _id: 'cross-file-canary-B', user: 'user-isolation', status: 'inbox', title: 'canary', createdTs: now, updatedTs: now });
    expect(await itemsDAO.findOne({ _id: 'cross-file-canary-B' })).not.toBeNull();
    expect(await itemsDAO.findOne({ _id: 'cross-file-canary-A' })).toBeNull();
});
